...
weather_Portland01 <- weather_Portland %>%
  mutate(RainToday = if_else(RainToday == "Yes", "1", RainToday), RainToday = if_else(RainToday == "No", "0", RainToday), RainToday = as.numeric(RainToday)) %>%
  mutate(RainTomorrow = if_else(RainTomorrow == "Yes", "1", RainTomorrow), RainTomorrow = if_else(RainTomorrow == "No", "0", RainTomorrow), RainTomorrow = as.numeric(RainTomorrow))
```
...
```{r}
weather_predlm <- predict(weather_fit1, test_data, type = "prob") %>%
  bind_cols(test_data)
weather_predlm %>%
  roc_curve(                      # plot ROC curve
    truth = RainTomorrow,
